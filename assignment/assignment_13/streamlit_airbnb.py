...
min_samples = st.sidebar.slider(
    "min_samples", min_value=2, max_value=10, step=1, value=3
)

st.sidebar.write("min_samples")
st.sidebar.write("#Create a slider for num_top_clusters")
num_top_clusters = st.sidebar.slider(
    "Number of Top Clusters to Show", min_value=1, max_value=10, step=1, value=5
)
...
bandwidth = st.sidebar.slider(
    "Bandwidth", min_value=0.001, max_value=0.020, step=0.001, value=0.005, format="%.3f"
)
...
try:
    ...
    with st.expander("View Cluster Statistics", expanded=False):
        for cluster_id in top_clusters.index:
            ...
            st.dataframe(sample_listings, width="stretch")
...
try:
    ...
    with st.expander("View Highest Density Locations", expanded=False):
        ...
        st.dataframe(top_density, width="stretch")
